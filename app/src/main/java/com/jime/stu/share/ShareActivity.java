...
import com.jime.stu.bean.ShareBean;
...
//            mShareBean = (ShareBean) getIntent().getSerializableExtra(TAG);
            ...
            if (data!=null){
                ...
                if(!TextUtils.isEmpty(title)){
                    mShareBean.setTitle(title);
                }
                String content = data.getQueryParameter("content");
                if(!TextUtils.isEmpty(content)){
                    mShareBean.setContent(content);
                }
                String url = data.getQueryParameter("url");
                if(!TextUtils.isEmpty(url)){
                    mShareBean.setUrl(url);
                }
            }else {
                mShareBean = (ShareBean) getIntent().getSerializableExtra(TAG);
            }
...
////                            }
                            ...
                            Bitmap bitmap =drawableToBitamp(getResources().getDrawable(R.mipmap.logo));

                            UMWeb web = new UMWeb(mShareUrl);
                            web.setTitle(mShareBean.getTitle());
                            web.setDescription(mShareBean.getContent());
...
//                    Toast.makeText(mActivity.get(),   " 分享成功啦", Toast.LENGTH_SHORT).show();
            ...
            if (handler!=null) {
                handler.sendEmptyMessageDelayed(FINSH, DELAY);
            }
